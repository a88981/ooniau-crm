// POST /api/setup  body: { "pageId": "你的 Notion Page ID" }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 4. 節日活動
    const holidaysId = await createDB(pageId, 'Ooniau 暖暖｜節日活動', {
      '名稱': { title: {} },
      '日期': { date: {} },
      '類型': { select: { options: [
        { name:'傳統節日', color:'yellow' },
        { name:'西洋節日', color:'blue'   },
        { name:'品牌活動', color:'purple' },
        { name:'其他',     color:'gray'   },
      ]}},
      '備忘': { rich_text: {} },
    });
